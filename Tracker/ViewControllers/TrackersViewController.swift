...
extension TrackersViewController: UICollectionViewDataSource {
        ...
        for category in categories {
            ...
            for tracker in category.trackerList {
                if let schedule = tracker.schedule {
                    ...
                    guard let recordsFromCurrentCell = try? trackerRecordStore.getTrackerRecords(with: tracker.id) else {
                        return []
                    }

                    if !recordsFromCurrentCell.isEmpty {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(
            withReuseIdentifier: TrackersCollectionViewCell.reuseIdentifier, for: indexPath
...
extension TrackersViewController: UICollectionViewDelegate {
    ...
    private func editTracker(indexPath: IndexPath) {
        ...
        if let _ = tracker.schedule {
            ...
            let viewController = CreateEventTrackerViewController(delegate: self, id: tracker.id, viewModel: viewModel)
    ...
    private func deleteTracker(indexPath: IndexPath) {
        ...
        let model = AlertModel(
            ...
            actions: [
                ...
                AlertActionModel(title: delete, style: .destructive) { [weak self] _ in
